...
# Y include the response of all thw sampels
...
def clean_reorder_data(x_feature):
    ...
    #print(x_feature['אבחנה-Histopatological degree'].unique())
    ...
    #'אבחנה-Histological diagnosis' #todo? delete for now, check hoe to do dummies
    #x_feature = pd.get_dummies(x_feature, columns=['אבחנה-Histological diagnosis'])
    #print(x_feature['אבחנה-Histological diagnosis'].unique())
...
def task_0(x_train, x_dev, labels_train, labels_dev, x_test):

    # re-write the labels to binary vector
    ...
    col_name = labels_dev.columns[0]
    ls_origin_labels = [eval(val) for val in labels_dev[col_name]]
    labels_dev_binary = [convert_y_to_binary_vector(resp) for resp in ls_origin_labels]


    # # basic estimator - LogisticRegression
    # clf = MultiOutputClassifier(LogisticRegression()).fit(x_train, labels_train_binary)
    ...
    # #k-NN
    # for k in range(1, 10):
    #     clf = MultiOutputClassifier(KNeighborsClassifier(k)).fit(x_train, labels_train_binary)
    #     pred_labels_dev = clf.predict(x_dev)
    #     pred_labels_dev_str = [[convert_binary_vector_to_y(resp)] for resp in pred_labels_dev]
    #     df = pd.DataFrame(pred_labels_dev_str)
    #     df.rename(columns={0: col_name}, inplace=True)
    ...
    #     clf = MultiOutputClassifier(DecisionTreeClassifier(max_depth=d)).fit(x_train, labels_train_binary)
    #     pred_labels_dev = clf.predict(x_dev)
    #     pred_labels_dev_str = [[convert_binary_vector_to_y(resp)] for resp in pred_labels_dev]
    #     df = pd.DataFrame(pred_labels_dev_str)
    #     df.rename(columns={0: col_name}, inplace=True)
    ...
    #     clf_forest = MultiOutputClassifier(RandomForestClassifier(num_classifiers, random_state=1)).fit(x_train, labels_train_binary)
    ...
    #     clf_ridge = MultiOutputClassifier(RidgeClassifier(lam)).fit(x_train, labels_train_binary)
    ...
    # deteemine best classifier
    # clf_knn = MultiOutputClassifier(KNeighborsClassifier(3)).fit(x_train, labels_train_binary)
    # pred_train_knn4 = clf_knn.predict(x_train)
    # pred_labels_train_str_knn = [[convert_binary_vector_to_y(resp)] for resp in pred_train_knn4]
    # df = pd.DataFrame(pred_labels_train_str_knn)
    # df.rename(columns={0: col_name}, inplace=True)
    # df.to_csv('pred_labels_train_str_knn_4.csv', index=False)
    #
    # clf_tree = MultiOutputClassifier(DecisionTreeClassifier(max_depth=14)).fit(x_train, labels_train_binary)
    # pred_train_tree14 = clf_tree.predict(x_train)
    # pred_labels_train_str = [[convert_binary_vector_to_y(resp)] for resp in pred_train_tree14]
    # df = pd.DataFrame(pred_labels_train_str)
    # df.rename(columns={0: col_name}, inplace=True)
    # df.to_csv('pred_labels_train_str_tree_4.csv', index=False)

    #best classifier, best micro (weighted)
...
def task_1(x_train, x_dev, labels_train, labels_dev):
    ...
    # cls_boost = GradientBoostingRegressor().fit(x_train, labels_train)
    ...
    # #SGDRegressor
    # cls_SGDR = SGDRegressor().fit(x_train, labels_train)
    ...
    # cls_elasticnet = ElasticNet(0.2).fit(x_train, labels_train)
...
if __name__ == '__main__':
    ...
    ## extracting the data to csv' for the evaluting
    ...
    task_0(x_train_c, x_dev_c, labels_0_train, labels_0_dev, x_test_c)
    task_1(x_train_c, x_dev_c, labels_1_train, labels_1_dev)

    # heatmap corr
    # df_corr = x_train_c.corr()
    #
    # fig = go.Figure()
